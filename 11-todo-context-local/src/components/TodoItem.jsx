        ...
        <div className={`flex border border-black/10 rounded-lg px-3 py-1.5 gap-x-3 
            ...
            ${todo.completed ? "bg-[#c6e9a7]":"bg-[#ccbed7]"}`}>
                ...
                <input type="text" className={
                    `border outline-none w-full bg-trnasparent rounded-lg 
                        ${isTodoEditable ? "border-black/10 px-2" : "border-transparent" }`}
                    value={todoMsg} readOnly={!isTodoEditable}
                    onChange={(e) => setTodoMsg(e.target.value)}
                />
                <button className='inline-flex w-8 h-8 rounded-lg text-sm border border-black/10
                    justify-content-center items-center bg-gray-50 hover:bg-gray-100 shrink-0 disabled:opacity-50'
                    onClick={() =>{
                        if(todo.completed){
                            return;
                        }
                    ...
                    >{isTodoEditable? "Save" : "Edit"}</button>
                     <button className='inline-flex w-8 h-8 rounded-lg text-sm border border-black/10
                        justify-content-center items-center bg-gray-50 hover:bg-gray-100 shrink-0'
